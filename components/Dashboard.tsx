    ...
    <div className="space-y-6 animate-fade-in pb-10">
          ...
          <div className="flex items-center gap-3">
              ...
              <div>
                  ...
                  <p className="text-xs text-blue-100 mt-1">
                      {settings.syncCode ? `Código: ${settings.syncCode}` : "Activa sincronización en configuración"}
                  </p>
              </div>
          </div>
      ...
      <div className="bg-blue-50 border border-blue-100 p-3 rounded-lg flex items-center justify-between no-print">
          <div className="flex items-center gap-2 text-blue-800 text-xs font-medium">
              <Apple className="w-4 h-4" /> Instalar en iPhone
          </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
            <h2 className="text-2xl font-bold text-slate-800">Panel</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <p className="text-sm font-medium text-gray-500">Ventas (Bruto)</p>
            <h3 className="text-2xl font-bold text-slate-800">{formatCurrency(totalSalesGross)}</h3>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <p className="text-sm font-medium text-gray-500">Gastos Reales</p>
            <h3 className="text-2xl font-bold text-slate-800">{formatCurrency(totalExpensesReal)}</h3>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <p className="text-sm font-medium text-gray-500">Ganancia Real</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b bg-slate-50 flex justify-between items-center">
                <h3 className="font-bold flex items-center gap-2"><Clock className="w-5 h-5" /> Activos Hoy</h3>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
             <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Users className="w-5 h-5" /> Agenda</h3>
             <div className="relative mb-4">
                <Search className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
                <input type="text" placeholder="Buscar..." className="w-full pl-9 py-2 border rounded-md" value={clientSearch} onChange={(e) => setClientSearch(e.target.value)} />
             </div>
